...
export async function createDatabase(request) {
  ...
  const { projectId, environmentId } = await getOrCreateDefaultEnv(Project, request.user.id)
  const envId = body.environmentId ?? environmentId
...
export async function listDatabases(request) {
  ...
  const services = await Service.find(query)
    .populate('serverId', 'name ip status userId')  // ← include userId for ownership filter
    .sort({ createdAt: -1 })
    .lean()

  // svc.serverId is the populated server object — svc.server does not exist
  const userServices = services.filter(svc =>
    svc.serverId?.userId?.toString() === request.user.id
  )
...
export async function getDatabaseStatus(request) {
  ...
  let liveStatus = null
  // service.serverId is the populated server — NOT service.server
  if (service.status === 'RUNNING' && service.containerName && service.serverId?.status === 'CONNECTED') {
    try {
      liveStatus = await getContainerStatus(buildServerConfig(service.serverId), service.containerName)
...
export async function getDatabaseStats(request) {
  ...
  const stats = await getContainerStats(buildServerConfig(service.serverId), service.containerName)
...
export async function getDatabaseLogs(request) {
  ...
  const logs = await getContainerLogs(buildServerConfig(service.serverId), service.containerName, tail)
...
export async function startDatabase(request) {
  ...
  const result = await startContainer(buildServerConfig(service.serverId), service.containerName)
  if (!result.success) return { success: false, message: 'Failed to start database' }

  await new Promise(r => setTimeout(r, 2000))
  const liveStatus = await getContainerStatus(buildServerConfig(service.serverId), service.containerName)
...
export async function stopDatabase(request) {
  ...
  const result = await stopContainer(buildServerConfig(service.serverId), service.containerName)
  if (result.success) {
    await Service.updateOne({ _id: id }, { $set: { status: 'STOPPED' } })
  }
...
export async function restartDatabase(request) {
  ...
  const result = await restartContainer(buildServerConfig(service.serverId), service.containerName)
...
export async function deleteDatabase(request, app) {
  ...
  if (service.containerName) {
    try {
      await removeContainer(buildServerConfig(service.serverId), service.containerName)
    } catch (err) {
...
export async function createBackupConfig(request) {
  ...
  const s3Test = await testS3Connection(buildServerConfig(service.serverId), s3TestCfg)
  if (!s3Test.ok) {
...
export async function testS3ConnectionHandler(request) {
  ...
  const result = await testS3Connection(buildServerConfig(service.serverId), {
