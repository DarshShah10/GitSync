...
export async function listSources(request) {
  const sources = await GitHubSource.find({ userId: request.user.id })
    ...
    .sort({ createdAt: -1 })
    .lean()
  return { success: true, data: sources.map(sanitizeSource) }
}

// ─── Get single source ─────────────────────────────────────────────────────────
export async function getSource(request) {
  const source = await GitHubSource.findOne({ _id: request.params.id, userId: request.user.id }).lean()
...
export async function createSource(request) {
  const { name, organization, isSystemWide, appId, installationId, clientId, clientSecret, webhookSecret, privateKey, htmlUrl, apiUrl, gitUser, gitPort } = request.body ?? {}
  if (!name) return { error: 'Name is required', status: 400 }
  const source = await GitHubSource.create({
    userId: request.user.id, name,
    organization: organization || null, isSystemWide: isSystemWide || false,
    appId: appId || null, installationId: installationId || null,
    clientId: clientId || null, clientSecret: clientSecret || null,
    webhookSecret: webhookSecret || null, privateKey: privateKey || null,
    htmlUrl: htmlUrl || 'https://github.com', apiUrl: apiUrl || 'https://api.github.com',
    gitUser: gitUser || 'git', gitPort: gitPort || 22,
...
//
// GitHub hits this endpoint in TWO separate redirects:
//   1. After app CREATION:  ?code=…&state=<sourceId>
//   2. After app INSTALL:   ?installation_id=…&setup_action=install   (NO state!)
//
export async function githubAppCallback(request, reply) {
  try {
    const { code, state: sourceId, installation_id, setup_action } = request.query ?? {}
    const frontendUrl = process.env.CLIENT_URL || 'http://localhost:5173'

    // ── Case: GitHub calling setup_url after the user installed the app on repos.
    // There is no `state` in this call, only `installation_id`.
    if (!sourceId && installation_id) {
      // Find the most recent source that still has no installationId (belongs to any user)
      const source = await GitHubSource.findOne({ installationId: null })
        .sort({ createdAt: -1 })

      if (source) {
        source.installationId = String(installation_id)
        source.isConnected = !!(source.appId && source.installationId && source.privateKey)
        await source.save()
        if (source.isConnected) {
          return reply.redirect(`${frontendUrl}/sources?connected=true&sourceId=${source._id}`)
        }
      }
      // Even if we couldn't match, send user back to sources page
      return reply.redirect(`${frontendUrl}/sources?connected=true`)
    }
    ...
    // Phase 2: capture installation_id if present
    if (installation_id) {
      source.installationId = String(installation_id)
    }

    // Auto-discover installation via API if we have credentials but still no installationId
    if (source.appId && source.privateKey && !source.installationId) {
      try {
        const appIdNumeric = parseInt(source.appId, 10)
        if (!isNaN(appIdNumeric)) {
          const installations = await fetchInstallations(appIdNumeric, source.privateKey, source.apiUrl || 'https://api.github.com')
...
export async function getSourceInstallations(request) {
  try {
    const source = await GitHubSource.findOne({ _id: request.params.id, userId: request.user.id }).lean()
    if (!source) return { error: 'Source not found', status: 404 }
    if (!source.appId || !source.privateKey) return { error: 'Source does not have credentials yet', status: 400 }
    const appIdNumeric = parseInt(source.appId, 10)
    if (isNaN(appIdNumeric)) return { error: 'App credentials not yet confirmed', status: 400 }
    const installations = await fetchInstallations(appIdNumeric, source.privateKey, source.apiUrl || 'https://api.github.com')
    return { success: true, data: installations.map(i => ({ id: i.id, account: { login: i.account?.login, type: i.account?.type } })) }
  } catch (err) {
...
export async function setInstallationId(request) {
  ...
  const source = await GitHubSource.findOne({ _id: request.params.id, userId: request.user.id })
  if (!source) return { error: 'Source not found', status: 404 }
...
export async function listSourceRepos(request) {
  try {
    ...
    const repos = await listInstallationRepos(source.appId, source.installationId, source.privateKey, source.apiUrl || 'https://api.github.com')
    return { success: true, data: repos.map(r => ({ fullName: r.full_name, isPrivate: r.private, defaultBranch: r.default_branch, description: r.description, htmlUrl: r.html_url })) }
  } catch (err) {
...
export async function updateSource(request) {
  const source = await GitHubSource.findOne({ _id: request.params.id, userId: request.user.id })
  if (!source) return { error: 'Source not found', status: 404 }
  const fields = ['name', 'organization', 'isSystemWide', 'appId', 'installationId', 'clientId', 'clientSecret', 'webhookSecret', 'privateKey', 'htmlUrl', 'apiUrl', 'gitUser', 'gitPort']
  const body = request.body ?? {}
  for (const field of fields) { if (body[field] !== undefined) source[field] = body[field] }
  source.isConnected = !!(source.appId && source.installationId && source.privateKey)
  await source.save()
  return { success: true, data: sanitizeSource(source.toObject()) }
}
...
export async function deleteSource(request) {
  const source = await GitHubSource.findOneAndDelete({ _id: request.params.id, userId: request.user.id })
  if (!source) return { error: 'Source not found', status: 404 }
