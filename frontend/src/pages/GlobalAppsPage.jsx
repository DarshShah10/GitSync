...
import { useQuery } from '@tanstack/react-query'
import { Layers, Plus, Trash2, Edit2, Globe, ChevronRight } from 'lucide-react'
      ...
      {isLoading ? (
        ...
        <EmptyShell>
          ...
          <div className="text-[var(--text-secondary)] mb-6 text-sm text-center max-w-[280px]">
            Deploy your first app from a Git repository, Dockerfile, or Docker image.
          </div>
          <button
            className="flex items-center gap-2 px-5 h-10 rounded-[var(--radius-md)]
                       bg-gradient-to-br from-[var(--primary)] to-[var(--primary-dim)]
...
function ProjectCard({ proj, onDelete, onDeploy }) {
  const navigate = useNavigate()

  return (
    <div className="
      bg-[var(--bg-elevated)] border border-[var(--border-ghost)]
      rounded-[var(--radius-xl)] p-6 flex flex-col
    ...
    ">
      ...
      <div className="flex justify-between items-start mb-6">
        <div className="flex gap-4 items-center">
          <div className="
            ...
            bg-[var(--bg-highest)] flex items-center justify-center
            border border-[var(--border-ghost)] text-[var(--secondary)]
          ">
            <Layers size={24} />
          </div>
          <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
          <span>Environments:</span>
          <span className="text-[var(--text-primary)] font-semibold">
            {proj.environments?.length ?? proj._count?.environments ?? 0}
          </span>
        </div>
      </div>
      ...
      <div className="flex gap-3 mt-auto border-t border-[var(--border-light)] pt-5">
        <button
          onClick={() => navigate(`/project/${proj._id ?? proj.id}`)}
          className="
        ...
        <button
          ...
          className="
            ...
            rounded-[var(--radius-md)] text-[var(--text-secondary)]
            text-[0.75rem] font-semibold transition-all
            hover:bg-[var(--bg-hover)] hover:text-[var(--primary)]
            hover:border-[var(--primary)]
          "
        >
          <ChevronRight size={14} /> New Service
        </button>
