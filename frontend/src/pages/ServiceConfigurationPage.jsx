...
// // ── Save Button — calls api.patch on click ───────────────────────────────────
...
// function DeployLogDrawer({ logs, onClose }) {
//   return (
//     <div className="fixed inset-x-0 bottom-0 z-50 h-72
//                     bg-[var(--bg-highest)] border-t border-[var(--border-ghost)]
//                     flex flex-col shadow-2xl">
//       <div className="flex items-center justify-between px-4 py-2 border-b border-[var(--border-ghost)]">
//         <span className="text-xs font-semibold text-[var(--text-secondary)] flex items-center gap-2">
//           <Terminal size={13} /> Deployment Logs
//         </span>
//         <button
//           onClick={onClose}
...
//         </button>
//       </div>
//       <div className="flex-1 overflow-y-auto p-4 font-mono text-xs text-emerald-400 leading-relaxed">
//         {logs.length === 0
...
//       internalPort,
//       envVars,
//       config: { branch, baseDir, buildPack },
//     })
//   }

//   // ── Deploy ────────────────────────────────────────────────────────────────
//   const handleDeploy = async () => {
//     // Guard: domain must be set before deploying
//     if (!domain.trim()) {
//       setDomainError(true)
...
//       // 2. Close any existing SSE stream
//       if (esRef.current) {
//         esRef.current.close()
//       }
...
//     // const es = new EventSource(`http://localhost:3000/api/deployments/${deploymentId}/logs`)
//       esRef.current = es

//       es.onmessage = (e) => {
//         const { output } = JSON.parse(e.data)
//         setDeployLogs(prev => [...prev, output])
//       }
...
//         esRef.current = null
//         setDeploying(false)
//         setDeployLogs(prev => [...prev, '⚠ Connection to log stream lost.'])
//       }
//     } catch (err) {
//       setDeploying(false)
//       setDeployLogs(prev => [...prev, `⚠ Deploy failed: ${err?.message ?? 'Unknown error'}`])
//     }
//   }
...
//               {/* ── Deploy button — wired to handleDeploy ── */}
//               <button
...
//                 </div>
//                 {/* ── Save button — passes handleSave callback ── */}
...
//                 {/* Domains — highlighted red border if domainError */}
//                 <div>
